...
ALTER view [AUR].[HRDC001_ApplicantPersonalAndContactPRFoundation_InsSel] as
...
SELECT DISTInCT T001.T001F005_EMPLOYEE_NO as EmployeeID
	 ...
	 , '' as SuffixType
	 , '' as Suffix
	 , CASE WHEN T000.T000F160_EMAIL_ADDRESS NOT LIKE '%uq.edu.au%' THEN 'email@gmail.com' ELSE '' END as HomeEmailAddress
	 , CASE WHEN T000.T000F160_EMAIL_ADDRESS NOT LIKE '%uq.edu.au%' THEN 'Y' ELSE 'N' END as HomeEmailPrimary
	 , CASE WHEN T000.T000F160_EMAIL_ADDRESS LIKE '%uq.edu.au%' THEN 'email@uq.edu.au' ELSE '' END as WorkEmailAddress
	 ...
	 , case when T000.T000F055_HOME_PHONE_NO is not null then '0799991111' else '' end as HomePhoneNumber 
     , '' as HomeExtension
     ...
     , case when T000.T000F045_CONTACT_PHONE_NO is not null then '0733651111' else '' end as WorkPhoneNumber    
	 , '' as WorkPhoneExtension
	 , case
		when T000.T000F045_CONTACT_PHONE_NO is not null  then 'BUSINESS_PHONE'
		else '' 
	   END
     ...
     , case when T000.T000F155_MOBILE_PHONE is not null then '0799991111' else '' end as AdditionalPhoneNumber 
	 , '' as AdditionalPhoneExtension
	 ,case
		when T000.T000F155_MOBILE_PHONE is not null then 'HOME'
		else '' 
	 ...
	 , case when T000.T000F320_CONTACT_MOBILE_PHONE is not null  then '0799991111' else '' end as Additional2PhoneNumber
